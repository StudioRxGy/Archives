...
if __name__=='__main__':
    ...
    try:
        # 使用新的Selenium API清理会话
        from selenium.webdriver.remote.remote_connection import RemoteConnection
        
        # 创建RemoteConnection实例（使用新的API）
        conn = RemoteConnection(Read_WEB_UI_Config().web_ui_config.selenium_hub, resolve_ip=False)
        
        # 获取所有会话
        sessions = conn.execute("GET", "/sessions")
        if sessions and 'value' in sessions:
            for session in sessions['value']:
                session_id = session['id']
                # 关闭会话
                conn.execute("DELETE", f"/session/{session_id}")
                
    except Exception as e:
        print('%s清除未关闭浏览器异常:\r\n%s'%(DateTimeTool.getNowTime(),str(e)))
