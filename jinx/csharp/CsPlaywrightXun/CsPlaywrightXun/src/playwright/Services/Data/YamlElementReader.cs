...
public class YamlElementReader
{
    ...
    /// <exception cref="ArgumentException">页面名称为空或无效时抛出</exception>
    ...
    /// <exception cref="FileNotFoundException">文件不存在时抛出</exception>
    /// <exception cref="YamlDataException">YAML数据格式错误时抛出</exception>
    public PageElementCollection LoadElementsFromPath(string filePath)
    {
    ...
    public PageElement GetElement(string pageName, string elementName)
    {
        ...
        var elementCollection = LoadElements(pageName);
    ...
    {
        var elementCollection = LoadElementsFromPath(filePath);
        return elementCollection.GetElement(pageName, elementName);
    }
