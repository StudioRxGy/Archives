...
public abstract class BaseApiTest : IDisposable
{
    ...
    /// <summary>
    /// 指示是否拥有 API 客户端的所有权（用于决定是否在释放时处理客户端）
    /// </summary>
    ...
    /// <param name="ownsApiClient">是否拥有API客户端的所有权，默认为false（由外部管理）</param>
    protected BaseApiTest(IApiClient apiClient, TestConfiguration configuration, ILogger logger, bool ownsApiClient = false)
    {
        ...
        Logger.LogInformation("BaseApiTest 初始化完成，环境: {Environment}, API基础URL: {ApiBaseUrl}, 拥有客户端: {OwnsClient}", 
            Configuration.Environment.Name, Configuration.Environment.ApiBaseUrl, _ownsApiClient);
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        {
            Logger.LogInformation("BaseApiTest 正在释放资源，拥有客户端: {OwnsClient}", _ownsApiClient);
            
            // 总是释放 API 服务
            ApiService?.Dispose();
            
            // 只有在拥有 API 客户端所有权时才释放它
            if (_ownsApiClient && ApiClient is IDisposable disposableClient)
            {
                disposableClient.Dispose();
                Logger.LogInformation("BaseApiTest 已释放拥有的 API 客户端");
            }
            else
            {
                Logger.LogInformation("BaseApiTest 不拥有 API 客户端，跳过客户端释放");
            }
