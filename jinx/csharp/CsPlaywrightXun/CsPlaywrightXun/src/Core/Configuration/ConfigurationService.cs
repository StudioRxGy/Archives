...
public class ConfigurationService
{
    ...
    {
        var configFilePath = PathConfiguration.GetEnvironmentConfigPath(environment);
    ...
    private TestConfiguration LoadConfigurationInternal(string environment)
    {
        ...
        // 添加基础配置文件（如果存在）
        var baseConfigPath = Path.Combine(PathConfiguration.EnvironmentsDirectory, "appsettings.json");
        ...
        var environmentConfigPath = PathConfiguration.GetEnvironmentConfigPath(environment);
        
        if (!File.Exists(environmentConfigPath))
        {
            throw new FileNotFoundException($"环境配置文件不存在: {environmentConfigPath}");
        }
