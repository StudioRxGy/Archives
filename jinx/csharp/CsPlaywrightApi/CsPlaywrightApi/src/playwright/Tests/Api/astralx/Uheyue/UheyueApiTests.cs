...
public class UheyueApiTests : IClassFixture<Fixture>
{
    ...
    {
        const string sourceFile = @"CsPlaywrightApi\src\playwright\Tests\Api\astralx\heyuye\UheyueApiTests.cs";
    ...
    {
        Login loginApi = new(_fixture.ApiContext!, _fixture.Logger);
        ...
        if (loginResponse.Headers.TryGetValue("set-cookie", out string? setCookieHeader))
        {
    ...
    public async Task Test01_Login_ShouldReturnSuccess()
    {
        // 设置测试上下文
        SetTestContext(
            "Test01_Login_ShouldReturnSuccess",
            "测试01 - 用户登录成功",
            "登录测试",
            ["API", "Login"],
            "High"
        ...
        // Act
        IAPIResponse response = await loginApi.AuthorizeUserAsync();
    ...
    public async Task Test02_Login_ShouldExtractToken()
    {
        // 设置测试上下文
        SetTestContext(
            "Test02_Login_ShouldExtractToken",
            "测试02 - 登录后能提取到Token",
            "登录测试",
            ["Login"],
            "High"
    ...
    public async Task Test03_CreateBtcOrder_ShouldSuccess()
    {
        // 设置测试上下文
        SetTestContext(
            "Test03_CreateBtcOrder_ShouldSuccess",
            "测试03 - 创建BTC市价买入订单",
            "市价买入测试",
            ["API", "Trade", "BuyOrder"],
            "High"
        ...
        // Arrange - 先登录获取 Token
        string? cToken = await LoginAndGetTokenAsync();
        Assert.NotNull(cToken);

        ShijiaBuy btcApi = new(_fixture.ApiContext!, _fixture.Logger);
        btcApi.SetCToken(cToken);

        // Act
        IAPIResponse response = await btcApi.CreateBtcOrderAsync();

        // Assert
        ApiAssertions.AssertSuccess(response);

        bool isSuccess = await btcApi.IsOrderCreatedSuccessfullyAsync(response);
        Assert.True(isSuccess, "订单创建失败");

        string? orderId = await btcApi.GetOrderIdFromResponseAsync(response);
    ...
    public async Task Test04_CreateBtcOrder_ShouldContainOrderId()
    {
        // 设置测试上下文
        SetTestContext(
            "Test04_CreateBtcOrder_ShouldContainOrderId",
            "测试04 - 验证订单响应包含必要字段",
            "市价买入测试",
            ["Trade", "Validation"],
            "Medium"
    ...
    public async Task Test05_ClosePosition_ShouldSuccess()
    {
        // 设置测试上下文
        SetTestContext(
            "Test05_ClosePosition_ShouldSuccess",
            "测试05 - 执行闪电平仓",
            "闪电平仓测试",
        ...
        Pingcang pingcangApi = new(_fixture.ApiContext!, _fixture.Logger);
        pingcangApi.SetCToken(cToken);

        // Act
        IAPIResponse response = await pingcangApi.CreateBtcOrderAsync();

        // Assert
        ApiAssertions.AssertSuccess(response);

        bool isSuccess = await pingcangApi.IsOrderCreatedSuccessfullyAsync(response);
        Assert.True(isSuccess, "平仓订单创建失败");

        string? orderId = await pingcangApi.GetOrderIdFromResponseAsync(response);
        Assert.NotNull(orderId);
    ...
    public async Task Test06_ClosePosition_ShouldContainOrderInfo()
    {
        // 设置测试上下文
        SetTestContext(
            "Test06_ClosePosition_ShouldContainOrderInfo",
            "测试06 - 验证平仓响应包含订单信息",
            "闪电平仓测试",
            ["Trade", "Validation"],
            "Medium"
    ...
    public async Task Test07_CompleteTradeFlow_ShouldSuccess()
    {
        // 设置测试上下文
        SetTestContext(
            "Test07_CompleteTradeFlow_ShouldSuccess",
            "测试07 - 完整交易流程（登录→开仓→平仓）",
            "完整流程测试",
            ["API", "E2E", "FullFlow"],
            "Critical"
        );
        ...
        IAPIResponse buyResponse = await btcApi.CreateBtcOrderAsync();
        ApiAssertions.AssertSuccess(buyResponse);

        string? buyOrderId = await btcApi.GetOrderIdFromResponseAsync(buyResponse);
        ...
        IAPIResponse closeResponse = await pingcangApi.CreateBtcOrderAsync();
        ApiAssertions.AssertSuccess(closeResponse);

        string? closeOrderId = await pingcangApi.GetOrderIdFromResponseAsync(closeResponse);
    ...
    public async Task Test08_CreateOrderWithoutToken_ShouldThrowException()
    {
        // 设置测试上下文
        SetTestContext(
            "Test08_CreateOrderWithoutToken_ShouldThrowException",
            "测试08 - 未设置Token时创建订单应失败",
            "异常场景测试",
            ["API", "Exception", "Negative"],
            "Medium"
        );
        ...
        // Act & Assert
        await Assert.ThrowsAsync<InvalidOperationException>(btcApi.CreateBtcOrderAsync);
    ...
    public async Task Test09_CreateOrderWithEmptyToken_ShouldThrowException()
    {
        // 设置测试上下文
        SetTestContext(
            "Test09_CreateOrderWithEmptyToken_ShouldThrowException",
            "测试09 - 使用空Token创建订单应失败",
            "异常场景测试",
            ["Exception", "Negative"],
            "Medium"
        );
