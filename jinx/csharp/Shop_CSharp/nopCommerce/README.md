...
### API 插件（api-for-nopcommerce）集成指引 ###

本仓库同级目录下包含 `api-for-nopcommerce/`，提供 Web API 插件方案：

- 插件本体：`api-for-nopcommerce/Nop.Plugin.Api`
- 构建脚本：`api-for-nopcommerce/build.ps1`（默认 Debug）
- 构建完成后，插件会自动输出到 `src/Presentation/Nop.Web/Plugins/Nop.Plugin.Api/`

快速使用：
1) 在 `api-for-nopcommerce/` 下执行：`pwsh -NoProfile -ExecutionPolicy Bypass -File .\build.ps1`
2) 回到本项目 `src/Presentation/Nop.Web` 启动站点：`dotnet run --configuration Debug`
3) 后台 `/admin` → Configuration → Plugins → Local plugins → 安装 “API Plugin”

如需更详细的安装与排错，请查看仓库根目录 `README.md`。
