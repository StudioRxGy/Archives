...
public class HtmlReportGenerator : IReportGenerator
{
    ...
    public async Task<string> GenerateReportAsync(TestReport testReport, string outputPath)
    {
        try
        {
            ...
            _logger.LogInformation("HTML报告生成完成: {OutputPath}", outputPath);
            return outputPath;
        }
        catch (Exception ex)
        {
    ...
    private string GenerateResultsSection(List<EnterpriseAutomationFramework.Core.Models.TestResult> results)
    {
        var sb = new StringBuilder();
    ...
    private string GenerateFailedTestsSection(List<EnterpriseAutomationFramework.Core.Models.TestResult> failedTests)
    {
        if (!failedTests.Any())
        {
